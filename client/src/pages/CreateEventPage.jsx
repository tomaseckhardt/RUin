...
import {
  ...
  getFilledInvitees,
  mergeInvitees,
} from "../lib/invitees.js";
import {
  clearSavedOrganizerToken,
  getSavedOrganizerEventIds,
} from "../lib/organizerLinkStorage.js";
import { getSavedOwner } from "../lib/ownerLinkStorage.js";
import {
  createEmptySignupPrefillItem,
  getFilledSignupPrefillItems,
} from "../lib/signupPrefillItems.js";

function parseTokenFromPath(path) {
  try {
    return (
      new URL(path, window.location.origin).searchParams.get("token") || ""
    );
  } catch {
    return "";
  }
...
function CreateEventPage() {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          toast.error(
            t("createEvent.afterpartyFailed", {
              error: afterpartyError.message,
            }),
          );
        }
      }
      ...
      if (filledInvitees.length > 0 && organizerToken) {
        try {
          ...
          toast.error(
            t("createEvent.invitesFailed", { error: inviteError.message }),
          );
        }
      }
      ...
      if (saveAsGroup && groupName.trim() && owner) {
        try {
          ...
          toast.error(
            t("createEvent.groupFailed", { error: groupError.message }),
          );
        }
      }
      ...
      if (saveAsTemplate && templateName.trim() && owner) {
        try {
          ...
          toast.error(
            t("createEvent.templateFailed", { error: templateError.message }),
          );
        }
      }
      ...
      if (form.enableBringList) {
        ...
        for (const item of filledBringItems) {
          try {
            ...
            toast.error(
              t("createEvent.bringItemFailed", {
                label: item.label,
                error: bringError.message,
              }),
            );
          }
        }
  ...
  function handleUseTemplate(template) {
    ...
    const defaultGroup = ownerPayload.groups.find(
      (group) => group.id === template.defaultGroupId,
    );
    ...
    async function loadRecentEvents() {
      ...
      try {
        ...
        const results = await Promise.allSettled(
          ids.map((eventId) => getEvent(eventId)),
        );
        const nextEvents = [];
  ...
  return (
    <PageShell
      ...
      <main className="grid gap-6 xl:grid-cols-[minmax(0,1.18fr)_minmax(360px,0.82fr)]">
        <section className="order-2 space-y-6 xl:order-1">
          <article className="panel relative overflow-hidden">
            ...
            <div className="relative grid gap-8 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] xl:items-end">
              <div>
                <p className="accent-copy text-sm font-semibold uppercase tracking-[0.28em]">
              ...
              <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-1">
                <div className="stat-tile">
                  ...
                  <div className="mt-2 text-3xl font-black tracking-[-0.04em] text-slate-950 dark:text-slate-50">
                    {t("createEvent.speedValue")}
                  </div>
                  <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                    {t("createEvent.speedText")}
                  </p>
                </div>
                <div className="stat-tile">
                  <div className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-300">
                  ...
                  <div className="mt-2 text-3xl font-black tracking-[-0.04em] text-slate-950 dark:text-slate-50">
                    {t("createEvent.flowValue")}
                  </div>
                  <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                    {t("createEvent.flowText")}
                  </p>
                </div>
                <div className="stat-tile">
                  <div className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-300">
                  ...
                  <div className="mt-2 text-3xl font-black tracking-[-0.04em] text-slate-950 dark:text-slate-50">
                    {t("createEvent.statusValue")}
                  </div>
                  <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                    {t("createEvent.statusText")}
                  </p>
                </div>
              </div>
            </div>
          ...
          <section className="grid gap-4 md:grid-cols-3">
            {whyItWorks.map((item, index) => (
              <article key={item} className="surface-subtle">
                <p className="accent-copy text-xs font-semibold uppercase tracking-[0.24em]">
                  0{index + 1}
                </p>
          ...
          <section className="grid gap-4 md:grid-cols-3">
            <article className="panel">
              ...
              <h3 className="mt-3 text-xl font-bold text-slate-950 dark:text-slate-50">
                {t("createEvent.stepShareTitle")}
              </h3>
              <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                {t("createEvent.stepShareText")}
              </p>
            </article>
            <article className="panel">
              <p className="accent-copy text-sm font-semibold uppercase tracking-[0.22em]">
              ...
              <h3 className="mt-3 text-xl font-bold text-slate-950 dark:text-slate-50">
                {t("createEvent.stepCollectTitle")}
              </h3>
              <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                {t("createEvent.stepCollectText")}
              </p>
            </article>
            <article className="panel">
              <p className="accent-copy text-sm font-semibold uppercase tracking-[0.22em]">
              ...
              <h3 className="mt-3 text-xl font-bold text-slate-950 dark:text-slate-50">
                {t("createEvent.stepDecideTitle")}
              </h3>
              <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
                {t("createEvent.stepDecideText")}
              </p>
            </article>
          </section>
        </section>
        ...
        <aside
          id="create-form"
          className="panel order-1 h-fit xl:order-2 xl:sticky xl:top-6">
          <div className="mb-6">
            <CollapsibleCard
              eyebrow={t("createEvent.recentEyebrow")}
              title={t("createEvent.recentTitle")}
              defaultOpen={false}>
              {isLoadingRecentEvents ? (
                <p className="text-sm text-slate-600 dark:text-slate-300">
                  {t("createEvent.recentLoading")}
                </p>
              ) : null}

              {!isLoadingRecentEvents && recentEvents.length === 0 ? (
                <p className="text-sm text-slate-600 dark:text-slate-300">
                  {t("createEvent.recentEmpty")}
                </p>
              ) : null}
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <EventDateTimePicker
                value={form.datetime}
                onChange={(nextValue) =>
                  setForm((current) => ({ ...current, datetime: nextValue }))
                }
              />
            </div>
            <div>
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-white/60 p-4 transition hover:border-fuchsia-200 dark:border-slate-700 dark:bg-slate-950/30">
              ...
              <div>
                <p className="text-sm font-medium text-slate-800 dark:text-slate-100">
                  {t("eventForm.requirePhone")}
                </p>
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                  {t("eventForm.requirePhoneHint")}
                </p>
              </div>
            </label>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-white/60 p-4 transition hover:border-fuchsia-200 dark:border-slate-700 dark:bg-slate-950/30">
                <input
                  ...
                  onChange={(e) =>
                    setForm((current) => ({
                      ...current,
                      enableBringList: e.target.checked,
                    }))
                  }
                />
                <div>
                  <p className="text-sm font-medium text-slate-800 dark:text-slate-100">
                    {t("eventForm.bringList")}
                  </p>
                  <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                    {t("eventForm.bringListHint")}
                  </p>
                </div>
              </label>
              <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-white/60 p-4 transition hover:border-fuchsia-200 dark:border-slate-700 dark:bg-slate-950/30">
                <input
                  ...
                  onChange={(e) =>
                    setForm((current) => ({
                      ...current,
                      enableCarpool: e.target.checked,
                    }))
                  }
                />
                <div>
            ...
            <CollapsibleCard
              eyebrow={t("createEvent.extrasEyebrow")}
              title={t("createEvent.extrasTitle")}
              defaultOpen={false}>
              <div className="space-y-4">
                {form.enableStops ? (
                  <div>
                    <button
                      ...
                      style={{
                        background:
                          "linear-gradient(135deg, #6f4cff, #a78bfa, #f472b6)",
                        animation: showAfterparty
                          ? "none"
                          : "party-pulse 1.8s ease-in-out infinite",
                      }}>
                      🎉{" "}
                      {showAfterparty
                        ? t("createEvent.afterpartyClose")
                        : t("createEvent.afterpartyOpen")}{" "}
                      🎉
                    </button>
                      ...
                      <div className="mt-3 grid gap-3 rounded-2xl border border-slate-200 bg-white/60 p-4 dark:border-slate-700 dark:bg-slate-950/30 sm:grid-cols-2">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              setAfterpartyLocation(event.target.value)
                            }
                            placeholder={t(
                              "createEvent.afterpartyLocationPlaceholder",
                            )}
                          />
                        </div>
                        <div>
                          ...
                          <input
                            ...
                            onChange={(event) =>
                              setAfterpartyTime(event.target.value)
                            }
                          />
                        </div>
                      </div>
                ...
                <div>
                  <button
                    ...
                    {showInvites
                      ? t("createEvent.invitesClose")
                      : t("createEvent.invitesOpen")}
                  </button>
                    ...
                    <div className="mt-3 space-y-4 rounded-2xl border border-slate-200 bg-white/60 p-4 dark:border-slate-700 dark:bg-slate-950/30">
                      ...
                      <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-white/60 p-4 transition hover:border-fuchsia-200 dark:border-slate-700 dark:bg-slate-950/30">
                        <input
                          ...
                          checked={saveAsGroup}
                          onChange={(event) =>
                            handleToggleSaveAsGroup(event.target.checked)
                          }
                          disabled={isSubmitting}
                        />
                        <div className="w-full">
                          <p className="text-sm font-medium text-slate-800 dark:text-slate-100">
                            {t("createEvent.saveAsGroup")}
                          </p>
                          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                            {t("createEvent.saveAsGroupHint")}
                          </p>
                          {saveAsGroup ? (
                            <input
                              ...
                              placeholder={t(
                                "createEvent.groupNamePlaceholder",
                              )}
                              disabled={isSubmitting}
                            />
                          ) : null}
                ...
                <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-white/60 p-4 transition hover:border-fuchsia-200 dark:border-slate-700 dark:bg-slate-950/30">
                  ...
                  <div className="w-full">
                    <p className="text-sm font-medium text-slate-800 dark:text-slate-100">
                      {t("createEvent.saveAsTemplate")}
                    </p>
                    <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                      {t("createEvent.saveAsTemplateHint")}
                    </p>
                    {saveAsTemplate ? (
                      <input
                        className="field mt-3"
