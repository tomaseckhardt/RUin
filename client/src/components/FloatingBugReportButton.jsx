    ...
    <>
      ...
      <ModalOverlay open={isOpen} onClose={close} labelledBy="feedback-title">
        <div className="w-full max-w-sm rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-900">
          ...
          <h3 id="feedback-title" className="mt-2 text-2xl font-black tracking-[-0.02em] text-slate-900 dark:text-slate-50">
            {copy('title')}
          </h3>
          ...
          <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
            <div>
              <label htmlFor="feedback-name" className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">
                {t('common.name')}
              </label>
              <input
                id="feedback-name"
            ...
            <div>
              <label htmlFor="feedback-message" className="mb-2 block text-sm font-medium text-slate-700 dark:text-slate-300">
                {copy('messageLabel')}
              </label>
              <textarea
                id="feedback-message"
                ...
                required
              />
              <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">{t('common.charactersLeft', { count: 2000 - message.length })}</p>
            </div>

            <div className="flex gap-3">
              <button type="button" className="secondary-button flex-1 justify-center" onClick={close} disabled={isSubmitting}>
                {t('common.cancel')}
              </button>
